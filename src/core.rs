...
use crate::{bus::Bus, cpu::CPU6502, io::IO, machine::FrameTimer, mem::Memory, ppu::Ppu, rng::Rng};
...
impl RetroCore for Core {
    fn init(env: &RetroEnvironment) -> Self {
        ...
        let frame_timer = FrameTimer::new(60.0, 2_000_000);
    ...
    fn run(&mut self, env: &RetroEnvironment, runtime: &RetroRuntime) {
        ...
        // 1 frame = 33,3333 cycles for 2Mhz/ 60 FPS
        for _ in 0..33_333 {
            self.emulator.clock();
        }
        ...
        for (idx, value) in framebuffer.iter().enumerate() {
            ...
            if *value != 0 {
                ...
                frame[frame_idx] = c as u8;
                frame[frame_idx + 1] = (c >> 4) as u8;
            } else {
                frame[frame_idx + 0] = 0;
                frame[frame_idx + 1] = 0;
            }
    ...
    fn load_game(&mut self, env: &RetroEnvironment, game: RetroGame) -> RetroLoadGameResult {
        match game {
            ...
            RetroGame::Data { meta, data } => {
                self.emulator.mem.load_mem(data, 0x600);
            }
            RetroGame::Path { meta, path } => {
                let data = std::fs::read(path).expect("Could not load file");
                self.emulator.mem.load_mem(&data, 0x600);
            }
        };
